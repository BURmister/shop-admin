...
export const fetchOneDelivery = createAsyncThunk('oneDelivery/fetchOneDelivery', async (id: string) => {
   const { data } = await axios.get(`http://localhost:3000/api/delivers/${id}`);
   return data;
});
